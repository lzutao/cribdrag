...
cribdrag, the interactive crib dragging script, allows you to interactively
decrypt ciphertext using a cryptanalytic technique known as "crib dragging".
This technique involves applying a known or guessed part of the plaintext (a
"crib") to every possible position of the ciphertext. By analyzing the result
of each operation and the likelihood of the result being a successful
decryption based on the expected format and language of the plaintext one can
recover the plaintext by making educated guesses and adaptive application of
the crib dragging technique.
''')
parser.add_argument('ciphertext', help='''
Ciphertext, encoded in an ASCII hex format (ie. ABC would be 414243)
''')
parser.add_argument('-c', '--charset', help='''
A regex-style character set to be used to identify best candidates for
successful decryption (ex: for alphanumeric characters and spaces,
use "a-zA-Z0-9 ")''', default='a-zA-Z0-9.,?! :;\'"')
args = parser.parse_args()

ctext = bytearray.fromhex(args.ciphertext)
ctext_len = len(ctext)
...
while response != 'end':
	...
	print_linewrapped(display_ctext, ctext_len)
	print("Your key is currently:")
	print_linewrapped(display_key, ctext_len)

	crib = raw_input("Please enter your crib: ")
	crib_len = len(crib)

	results = sxor(ctext, ctext_len, bytearray(crib), crib_len)
	results_len = len(results)

	#Generate results
	for result_index in xrange(results_len):
		if (re.search(CHARSET,results[result_index])):
			print('*** %5d: %r'%(result_index, results[result_index]))
		else:
			print('    %5d: %r'%(result_index, results[result_index]))
